...
rule filter_variants:
    input:
        ...
        biallelic_vcf=f"{data_dir}/caes_biallelic_snps.vcf"
    output:
        filtered_vcf=f"{data_dir}/caes_biallelic_snps_qualfilter.vcf"
    shell:
        """
        module load GATK/4.4.0.0-GCCcore-12.3.0-Java-17
...
rule extract_passed_variants:
    input:
        filtered_vcf=f"{data_dir}/caes_biallelic_snps_qualfilter.vcf"
    output:
        filtered_passed_vcf=f"{data_dir}/caes_biallelic_snps_qualfilterPASSED.vcf"
    shell:
        """
...
rule filter_invariants:
    input:
        ref=f"{ref_dir}/{ref}",
        invcf=f"{data_dir}/caes_invariant.vcf"
    output:
        filtered_invcf=f"{data_dir}/caes_invariant_qualfilter.vcf"
    shell:
        """
        module load GATK/4.4.0.0-GCCcore-12.3.0-Java-17
        gatk VariantFiltration \
...
rule extract_passed_invariants:
    input:
        filtered_invcf=f"{data_dir}/caes_invariant_qualfilter.vcf"
    output:
        filtered_passed_invcf=f"{data_dir}/caes_invariant_qualfilterPASSED.vcf"
    shell:
        """
